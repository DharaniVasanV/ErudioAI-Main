...
export const AnalyticsPage = () => {
   const { subjects, user, topics } = useApp();
   const navigate = useNavigate();

   // Load real data from localStorage
   const studyTimeSeconds = parseInt(localStorage.getItem('study_time_seconds') || '0');
   ...
   const quizAvg = quizResults.length > 0
      ? Math.round(quizResults.reduce((acc: number, r: any) => acc + r.score, 0) / quizResults.length)
      : 0;

   const progressData = subjects.map(s => ({
      name: s.name,
      progress: s.progress,
      color: s.color.replace('bg-', 'text-').replace('-500', '')
   }));
   ...
   const pieData = [
      { name: 'Mastered', value: mastered || 1 }, // Avoid 0 for pie chart display if empty
      ...
      <div className="space-y-6">
         ...
         <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm flex flex-col items-center text-center">
               ...
               <p className="text-2xl font-bold text-slate-900">{studyTimeHours}h</p>
            ...
            <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm flex flex-col items-center text-center">
               ...
               <p className="text-2xl font-bold text-slate-900">{quizAvg}%</p>
