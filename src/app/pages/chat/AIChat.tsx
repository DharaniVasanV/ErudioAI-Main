...
import { Send, Mic, User, Bot, Paperclip, Copy, Check, ArrowLeft } from 'lucide-react';
...
export const AIChat = () => {
  ...
  // Load past messages if chatId provided
  useEffect(() => {
    if (chatId) {
      const loadMessages = async () => {
        ...
        try {
          ...
          const res = await fetch(`${config.api.baseUrl}/chat/${chatId}/messages`, {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // Build the message list to send (all current messages + new one)
      ...
      // Only record the first message of a brand new conversation
      if (!isExistingChat && messages.length <= 1) {
        addRecentActivity({
  ...
  return (
    <div className="h-[calc(100vh-100px)] flex flex-col bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            ...
            >
              ...
              {msg.role === 'assistant' && msg.showSuggestion && msg.suggestedTopic && (
                <div className="ml-12 mt-1 bg-indigo-50 border border-indigo-100 rounded-xl px-4 py-3 text-xs text-slate-700 flex flex-wrap items-center gap-2">
                  <span>
                    Add "<span className="font-semibold">{msg.suggestedTopic}</span>" to your lessons &amp; timetable?
                  </span>
      ...
      <div className="p-4 bg-white border-t border-slate-100">
        {messages.length <= 1 && !isExistingChat && (
