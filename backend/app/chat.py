...
async def chat(
    ...
    if hf_api_url and hf_api_url.strip():
        try:
            print(f"Attempting to use Custom HF API: {hf_api_url}")
            full_text = call_custom_hf_chat(hf_api_url, prompt)
            model_source = "Colab/Llama-3.2"
            print("✅ Success: Response generated by Custom Colab Model")
        except Exception as e:
    ...
    if not full_text:
        ...
        for model_name in models_to_try:
            try:
                ...
                full_text = resp.text
                if full_text:
                    model_source = f"Gemini ({model_name})"
                    print(f"✅ Success: Response generated by {model_source}")
                    break
            except Exception as e:
                last_error = e
                continue
    ...
    # Prepend source debug info for the user (optional, can be removed later)
    # clean_text = f"[{model_source}] {clean_text}" 

    print(f"Final Model Source: {model_source}")
